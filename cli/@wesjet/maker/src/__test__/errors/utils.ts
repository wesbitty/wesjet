...
import { singleItem, unknownToRelativePosixFilePath } from "@wesjet/utils";
...
export const makeSchemaDef = (): core.SchemaDef => {
  ...
  const schemaDef: core.SchemaDef = {
    hash: "",
    nestedTypeDefMap: {},
    documentTypeDefMap: { TypeA, TypeB },
  };
  return schemaDef;
};
  ...
  doNTimes(countRecord.ExtraFieldDataError, (index) => {
    ...
    errors.push(
      new FetchDataError.ExtraFieldDataError({
        ...
        documentTypeDef,
        extraFieldEntries: [
          index === 0 ? ["someKey", "someVal"] : ["someOtherKey", 42],
        ],
      })
    );
  });
