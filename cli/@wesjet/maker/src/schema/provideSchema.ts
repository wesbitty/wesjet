...
export const makeCoreSchema = ({
  ...
  T.gen(function* ($) {
    ...
    for (const documentDef of documentTypeDefs) {
      ...
      if (
        fieldDefs.some((_) => _.name === options.fieldOptions.bodyFieldName)
      ) {
        yield* $(
          T.fail(
            new DuplicateBodyFieldError({
              bodyFieldName: options.fieldOptions.bodyFieldName,
            })
          )
        );
      }
      ...
      if (
        documentDef.contentType === undefined ||
        documentDef.contentType === "markdown"
      ) {
        fieldDefs.push({
          type: "markdown",
...
const getFieldDefEntries = (
  fieldDefs: LocalSchema.FieldDefs
): FieldDefEntry[] => {
...
const getFieldDefValues = (
  fieldDefs: LocalSchema.FieldDefs
): LocalSchema.FieldDef[] => {
  if (Array.isArray(fieldDefs)) {
    return fieldDefs;
  } else {
  ...
  switch (fieldDef.type) {
    case "list":
      if (LocalSchema.isListPolymorphicFieldDef(fieldDef)) {
        return identity<core.ListPolymorphicFieldDef>({
          ...
          of: fieldDef.of.map((_) =>
            fieldListItemsToCoreFieldListDefItems(_, fieldOptions)
          ),
        });
      }
    ...
    case "nested":
      if (LocalSchema.isNestedPolymorphicFieldDef(fieldDef)) {
        ...
        const containsUnnamedTypeDef = nestedTypeDefs.some(
          LocalSchema.isNestedUnnamedTypeDef
        );
        if (containsUnnamedTypeDef) {
          throw new Error(
            `Nested unnamed polymorphic type definitions are not yet supported.`
          );
        }
        const nestedTypeNames = nestedTypeDefs.map(
          (_: any) => _.name as string
        );
      ...
      const fieldDefs = getFieldDefEntries(nestedTypeDef.fields ?? []).map(
        (_) => fieldDefEntryToCoreFieldDef(_, fieldOptions)
      );
...
const collectNestedDefs = (
      ...
      case "nested":
        if (utils.isReadonlyArray(fieldDef.of)) {
          ...
          return nestedTypeDefs.forEach((nestedTypeDef) => {
            ...
            return getFieldDefValues(nestedTypeDef.fields ?? []).forEach(
              traverseField
            );
          });
        }
  ...
  documentDefs
    .flatMap((_) => getFieldDefValues(_.fields ?? []))
    .forEach(traverseField);
