...
import { O, OT, pipe, T, These } from "@wesjet/utils/effect";
    ...
    T.gen(function* ($) {
      ...
      yield* $(
        DocumentTypeMapState.update((_) =>
          _.add(documentDefName, relativeFilePath)
        )
      );
      ...
      const requiredFieldsWithoutDefaultValue =
        documentTypeDef.fieldDefs.filter(
          (_) =>
            _.isRequired && _.default === undefined && _.isSystemField === false
        );
      ...
      const schemaFieldNames = documentTypeDef.fieldDefs
        .map((_) => _.name)
        .concat([typeFieldName]);
...
const getDocumentDefName = ({
  ...
  return getDocumentDefNameByFilePathPattern({
    filePathPatternMap,
    relativeFilePath,
  });
};
...
const getDocumentDefNameByFilePathPattern = ({
  ...
  const matchingFilePathPattern = Object.keys(filePathPatternMap).find(
    (filePathPattern) =>
...
const validateContentTypeMatchesFileExtension = ({
  ...
  const isInvalidData =
    contentType === "data" && validDataExtensions.includes(extension) === false;
