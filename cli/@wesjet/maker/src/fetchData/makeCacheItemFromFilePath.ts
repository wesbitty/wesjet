...
import { identity, O, OT, pipe, T, These } from "@wesjet/utils/effect";
  ...
  pipe(
    T.gen(function* ($) {
      ...
      if (
        ...
        previousCache.cacheItemsMap[relativeFilePath]!.documentHash ===
          documentHash &&
        ...
        yield* $(
          DocumentTypeMapState.update((_) =>
            _.add(cacheItem.documentTypeName, relativeFilePath)
          )
        );
      ...
      const rawContent = yield* $(
        processRawContent({ fullFilePath, relativeFilePath })
      );
    ...
    T.mapError((error) => {
      switch (error._tag) {
        ...
        case "node.fs.FileNotFoundError":
          return new FetchDataError.UnexpectedError({
            error,
            documentFilePath: relativeFilePath,
          });
        default:
...
> =>
  pipe(
    T.gen(function* ($) {
      ...
      const filePathExtension = relativeFilePath
        .toLowerCase()
        .split(".")
        .pop()!;
      ...
      switch (filePathExtension) {
        ...
        default:
          return yield* $(
            T.fail(
              new FetchDataError.UnsupportedFileExtension({
...
const getComputedValues = ({
  ...
  unknown,
  FetchDataError.ComputedValueError,
  ...
  T.tryCatch(
    ...
    (error: any) => {
      if (error.name === "YAMLException") {
        return new FetchDataError.InvalidFrontmatterError({
          error,
          documentFilePath,
        });
      } else {
        return new FetchDataError.InvalidMarkdownFileError({
          error,
          documentFilePath,
        });
      }
...
> =>
  T.tryCatch(
    ...
    (error) =>
      new FetchDataError.InvalidJsonFileError({ error, documentFilePath })
  );
...
> =>
  T.tryCatch(
    ...
    (error) =>
      new FetchDataError.InvalidYamlFileError({ error, documentFilePath })
  );
