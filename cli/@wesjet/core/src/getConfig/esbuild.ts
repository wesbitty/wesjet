...
class ConcreteEsbuildWatcher implements EsbuildWatcher {
  ...
  constructor(
    ...
    private fsEventsHub: H.Hub<
      Ex.Exit<never, E.Either<EsbuildError, esbuild.BuildResult>>
    > // public readonly paths: readonly string[], // public readonly options?: Chokidar.WatchOptions
  ) {}
  ...
  shutdown: T.Effect<unknown, never, void> = pipe(
    ...
    T.chain((initialBuildResult) => {
      ...
      console.log(
        `This shouldn't happen. Seems like esbuild watcher wasn't running (yet).`
      );
      return T.unit;
    }),
    T.catchAll((_) => T.unit)
  ...
  start: T.Effect<OT.HasTracer, never, void> = pipe(
            ...
            onRebuild: (error, result) => {
              if (error) {
                T.run(
                  H.publish_(
                    this.fsEventsHub,
                    Ex.succeed(E.left(new KnownEsbuildError({ error })))
                  )
                );
              } else {
                T.run(
                  H.publish_(this.fsEventsHub, Ex.succeed(E.right(result!)))
                );
              }
            },
          },
    ...
    OT.withSpan("esbuild", {
      attributes: { buildOptions: JSON.stringify(this.buildOptions) },
    }),
    T.tap((initialBuildResult) =>
      Ref.set_(this.initialBuildResult, O.some(initialBuildResult))
    ),
    T.tap((initialBuildResult) =>
      H.publish_(this.fsEventsHub, Ex.succeed(E.right(initialBuildResult)))
    ),
    T.catchAll((error) =>
      H.publish_(this.fsEventsHub, Ex.succeed(E.left(error)))
    )
  );
  ...
  subscribe: M.Managed<
    unknown,
    never,
    S.Stream<unknown, never, E.Either<EsbuildError, esbuild.BuildResult>>
  > = pipe(
    H.subscribe(this.fsEventsHub),
    M.chain((_) =>
      M.ensuringFirst_(M.succeed(S.fromQueue()(_)), Q.shutdown(_))
    ),
    M.map(S.flattenExit)
  );
}
...
function concrete(
  esbuildWatcher: EsbuildWatcher
): asserts esbuildWatcher is ConcreteEsbuildWatcher {
  //
}

export const make = (
  buildOptions: esbuild.BuildOptions
): T.Effect<unknown, never, EsbuildWatcher> =>
  pipe(
    ...
    T.chain(({ tuple: [initialBuildResult, hub] }) =>
      T.succeedWith(
        () => new ConcreteEsbuildWatcher(initialBuildResult, buildOptions, hub)
      )
    )
    // T.tap((_) => _.start),
  );
...
export const start = (
  self: EsbuildWatcher
): T.Effect<OT.HasTracer, never, void> => {
  concrete(self);
