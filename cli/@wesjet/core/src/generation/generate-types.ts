...
export type DocumentTypeNames = ${renderUnion(
    documentTypeNames.map(wrapInQuotes)
  )}

export type NestedTypes = ${
    nestedTypes.length > 0 ? renderUnion(nestedTypeNames) : "never"
  }
export type NestedTypeNames = ${
    nestedTypes.length > 0
      ? renderUnion(nestedTypeNames.map(wrapInQuotes))
      : "never"
  }
...
export const renderDocumentTypeDefOrNestedTypeDef = ({
  ...
  const computedFields = (
    def._tag === "DocumentTypeDef" ? def.computedFields : []
  )
    .map(
      (field) =>
    ...
    .join("\n");
  const description =
    def.description ?? def.extensions.stackbit?.fields?.[def.name]?.label;
...
const renderFieldDef = (field: FieldDef): string => {
  const canBeUndefined =
    field.isRequired === false && field.default === undefined;
  return `${field.description ? `  /** ${field.description} */\n` : ""}  ${
    field.name
  }${canBeUndefined ? "?" : ""}: ${renderFieldType(field)}${
...
const renderUnion = (typeNames: readonly string[]): string =>
  typeNames.join(" | ");

const renderPolymorphicListType = (typeNames: string[]): string =>
  wrapInParenthesis(renderUnion(typeNames)) + "[]";

const indent = (spaceCount: number) => (str: string) =>
  `${" ".repeat(spaceCount)}${str}`;
