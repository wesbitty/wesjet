...
export const validateSchema = (schema: SchemaDef): void => {
  Object.values(schema.documentTypeDefMap).forEach((def) =>
    validateDocumentOrObjectDef({ def })
  );
  Object.values(schema.nestedTypeDefMap).forEach((def) =>
    validateDocumentOrObjectDef({ def })
  );
};

const validateDocumentOrObjectDef = ({
  def,
}: {
  ...
  if (stackbitExt?.labelField) {
    const noFieldFoundForLabelField = !def.fieldDefs.some(
      (_) => _.name === stackbitExt.labelField
    );
