...
export const fetchAllDocuments = ({
  ...
  core.SourceFetchDataError,
  core.DataCache.Cache
> =>
  pipe(
    T.gen(function* ($) {
      const environment = yield* $(
        getEnvironment({ accessToken, spaceId, environmentId })
      );
      ...
      if (process.env["CL_DEBUG"]) {
        yield* $(
          OT.addAttribute("schemaOverrides", JSON.stringify(schemaOverrides))
        );
        yield* $(
          fs.writeFileJson({
            filePath: ".tmp.assets.json",
            content: allAssets as any,
          })
        );
        yield* $(
          fs.writeFileJson({
            filePath: ".tmp.entries.json",
            content: allEntries as any,
          })
        );
      }
      ...
      const cacheItemsMap = Object.fromEntries(
        Chunk.map_(documents, (_) => [_.document._id, _])
      );
    ...
    T.mapError(
      (error) => new core.SourceFetchDataError({ error, alreadyHandled: false })
    )
  );
...
const getAllEntries = (
  ...
  pipe(
    T.gen(function* ($) {
      ...
      const { total } = yield* $(
        environmentGetEntries({ limit: 0, environment })
      );
      const chunkSize = 500;
...
const getAllAssets = (
  ...
  pipe(
    T.gen(function* ($) {
      ...
      const { total } = yield* $(
        environmentGetAssets({ limit: 0, environment })
      );
      const chunkSize = 500;
