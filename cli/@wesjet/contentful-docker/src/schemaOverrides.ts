...
export namespace Input {
  ...
  export type TypeOverrideValues =
    | (string | TypeOverrideMap)[]
    | TypeOverrideMap;

  /** Map from Contentful content type id to your own type name */
  export type ContentTypeId = string;
  export type TypeOverrideMap = Record<
    ContentTypeId,
    string | TypeOverrideItem
  >;

  export type TypeOverrideItem = {
    defName: string;
    isSingleton?: boolean;
    fields?: FieldOverrideMap;
  };
...
export const normalizeSchemaOverrides = ({
  ...
  const overrideDocumentContentTypeIds = getContentTypeIds(
    schemaOverrides.documentTypes ?? {}
  );
  const overrideNestedContentTypeIds = getContentTypeIds(
...
`);
    ...
    const duplicateContentTypeIds = overrideDocumentContentTypeIds.filter(
      (typeName) => overrideNestedContentTypeIds.includes(typeName)
    );
...
const normalizeInputTypeValues = (
  types: Input.TypeOverrideValues
    ...
    return types.reduce<Normalized.TypeOverrideMap>(
      (acc, type_) =>
        typeof type_ === "object"
          ...
          : {
              ...acc,
              [type_]: { defName: type_, isSingleton: false, fields: {} },
            },
      {}
    );
  }
...
const normalizeInputTypeOverrideMap = (
  ...
  mapObjectValues(mapping, (_key, item) =>
    normalizeInputTypeOverrideItem(item)
  );
...
const getContentTypeIds = (types: Input.TypeOverrideValues): string[] => {
  if (Array.isArray(types)) {
    return types.flatMap((type_) =>
      typeof type_ === "object" ? Object.keys(type_) : [type_]
    );
  }
...
const getCompilementTypeNames = ({
  ...
  return allTypeNames.filter(
    (typeName) => !providedTypeNames.includes(typeName)
  );
};
