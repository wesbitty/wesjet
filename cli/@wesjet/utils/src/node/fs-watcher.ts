...
class ConcreteFileWatcher extends FileWatcherInternal {
  ...
  constructor(
    ...
    private fsEventsHub: H.Hub<
      Ex.Exit<never, E.Either<FileWatcherError, FileSystemEvent>>
    >,
          ...
          _.on("error", (error) => {
            T.run(
              H.publish_(
                this.fsEventsHub,
                Ex.succeed(
                  E.left(new FileWatcherError({ origin: O.some(error) }))
                )
              )
            );
          });
                ...
                T.run(
                  H.publish_(
                    ...
                    Ex.succeed(
                      E.right(
                        new FileAdded(
                          unknownFilePath(path),
                          O.fromNullable(stats)
                        )
                      )
                    )
                  )
...
): asserts fileWatcher is ConcreteFileWatcher {
  //
}
...
export function makeUnsafe(
  ...
  const instance = Ref.unsafeMakeRef<Chokidar.FSWatcher>(
    Chokidar.watch(paths, options)
  );
  const hub =
    H.unsafeMakeUnbounded<
      Ex.Exit<never, E.Either<FileWatcherError, FileSystemEvent>>
    >();
...
export function make(
  ...
  return pipe(
    ...
    T.chain(({ tuple: [instance, hub] }) =>
      T.succeedWith(
        () => new ConcreteFileWatcher(instance, hub, paths, options)
      )
    ),
    T.tap((_) => _.subscribeToEvents())
  );
...
export const makeAndSubscribe = (
  ...
  pipe(
    M.make_(make(paths, options), shutdown),
    M.chain(subscribe),
    S.unwrapManaged
  );
