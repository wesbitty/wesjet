...
export type AbsolutePosixFilePath = Branded.Branded<
  string,
...
export const absolutePosixFilePath = (path_: string): AbsolutePosixFilePath => {
  ...
  if (!path.isAbsolute(path_)) {
    throw new Error(
      `Expected an absolute path (i.e. starting with '/' or '\\'), got ${path_}`
    );
  }
...
export type RelativePosixFilePath = Branded.Branded<
  string,
...
export const relativePosixFilePath = (path_: string): RelativePosixFilePath => {
  ...
  if (path.isAbsolute(path_)) {
    throw new Error(
      `Expected a relative path (i.e. not starting with '/' or '\\'), got ${path_}`
    );
  }
...
export const isPosixFilePathString = (path_: string) =>
  !path_.includes(path.win32.sep);
...
export const unknownToRelativePosixFilePath = (
  ...
  return relativePosixFilePath(
    path_.split(path.win32.sep).join(path.posix.sep)
  );
};
...
export const unknownFilePath = (path_: string): UnknownFilePath =>
  Branded.makeBranded(path_);

export function filePathJoin(
  ...paths: RelativePosixFilePath[]
): RelativePosixFilePath;
export function filePathJoin(
  ...paths: [AbsolutePosixFilePath, ...string[]]
): AbsolutePosixFilePath;
export function filePathJoin(
  ...paths: string[]
): RelativePosixFilePath | AbsolutePosixFilePath {
