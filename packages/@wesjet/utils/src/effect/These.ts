...
export const isNonFailure = <E, A>(
  self: These<E, A>
): self is These<never, A> => E.isRight(self.either);
  ...
  return new These(
    E.fold_(
      ...
      (
        ...
        E1 | E2 | E3,
        Tp.Tuple<[A1 | A2 | A3, O.Option<E1 | E2 | E3>]>
      > => onFail(x).either,
      ({ tuple: [result, warnings] }) =>
        warnings._tag === "None"
          ? onSuccess(result).either
          : onBoth(result, warnings.value).either
    )
  );
}
...
export function mapError_<E0, E, A>(
  self: These<E0, A>,
  f: (a: E0) => E
): These<E, A> {
  return foldM_(
...
export function chain<E0, A0, E, A>(
  f: (a: A0, w: O.Option<E0>) => These<E, A>
) {
...
export const effectUnwrapValue = <T, E1, E2, A>(
  ...
  return pipe(
    effect,
    T.chain((these) =>
      E.fold_(these.either, T.fail, ({ tuple: [a] }) => T.succeed(a))
    )
  );
};
...
export const effectTapSuccess =
  ...
  (
    effect: T.Effect<T2, E2, These<TE2, A>>
  ): T.Effect<T1 & T2, E1 | E2, These<TE2, A>> => {
