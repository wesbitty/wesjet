...
export const fileOrDirExists = (
  pathLike: string
): T.Effect<OT.HasTracer, StatError, boolean> => {
  return pipe(
    stat(pathLike),
...
export const symlinkExists = (
  pathLike: string
): T.Effect<unknown, StatError, boolean> => {
  return pipe(
    stat(pathLike),
...
export const stat = (
  filePath: string
): T.Effect<unknown, FileNotFoundError | StatError, Stats> => {
...
export const readFileJson = <T extends JsonValue = JsonValue>(
  ...
  OT.HasTracer,
  ReadFileError | FileNotFoundError | JsonParseError,
  T
> =>
  pipe(
    readFile(filePath),
...
export const readFileJsonIfExists = <T extends JsonValue = JsonValue>(
  ...
  OT.HasTracer,
  StatError | ReadFileError | JsonParseError,
  T | undefined
> =>
  pipe(
    fileOrDirExists(filePath),
    T.chain((exists) =>
      exists ? readFileJson<T>(filePath) : T.succeed(undefined)
    ),
...
export const mkdirp = <T extends string>(
  dirPath: T
): T.Effect<OT.HasTracer, MkdirError, void> =>
...
export class FileOrDirNotFoundError extends Tagged(
  "node.fs.FileOrDirNotFoundError"
)<{
...
export class UnknownFSError extends Tagged("node.fs.UnknownFSError")<{
  ...
  toString = () =>
    `UnknownFSError: ${errorToString(this.error)} ${this.error.stack}`;
}
