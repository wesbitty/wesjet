...
export const stackbitDocumentLikeModelToDocumentType =
  ...
  (
    ...
    return SourceFiles.defineDocumentType(() => ({
      ...
      isSingleton:
        stackbitModel.type === "config" ||
        stackbitModel.singleInstance === true,
    }));
  };
...
const stackbitFieldToField =
  ...
  (stackbitField: Stackbit.Field): SourceFiles.FieldDefWithName => {
    ...
    switch (stackbitField.type) {
      ...
      case "number":
        type FieldDef =
          | SourceFiles.BooleanFieldDef
          | SourceFiles.NumberFieldDef;
      ...
      case "reference": {
        const of = stackbitField.models.map(
          (modelName) => ctx.documentTypeMap[modelName]!
        );
        if (of.length === 1) {
          return identity<WithName<SourceFiles.ReferenceFieldDef>>({
            ...commonFields,
            type: "reference",
            of: of[0]!,
          });
      ...
      case "model": {
        const of = stackbitField.models.map(
          (modelName) => ctx.nestedTypeMap[modelName]!
        );
        if (of.length === 1) {
      ...
      case "markdown":
        return identity<WithName<SourceFiles.MarkdownFieldDef>>({
          ...commonFields,
          type: "markdown",
        });
      case "json":
...
const stackbitListItemToListFieldDef =
  ...
  (
    ...
    | SourceFiles.ListFieldDefItem.Item
    | readonly SourceFiles.ListFieldDefItem.Item[] => {
    switch (stackbitListItem.type) {
      ...
      case "model":
        return firstArrayItemIfOne(
          stackbitListItem.models.map((modelName) =>
            identity<SourceFiles.ListFieldDefItem.ItemNestedType>(
              ctx.nestedTypeMap[modelName]!
            )
          )
        );
      case "object":
