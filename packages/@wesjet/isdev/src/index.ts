...
>(
  ...
  overrideArgs: WesjetOverrideArgs<TDocumentTypeNames> = {
    documentTypes: {},
    nestedTypes: {},
  }
...
>(
  ...
  const [documentLikeModels, objectModels] = partition(
    models,
    isDocumentLikeModel
  );
  ...
  const documentTypes = documentLikeModels.map(
    stackbitDocumentLikeModelToDocumentType(ctx)
  );
  ...
  objectModels.forEach((model) => {
    ...
    if (nestedOverride?.fields && fields) {
      for (const [fieldName, { type }] of Object.entries(
        nestedOverride.fields
      )) {
  ...
  documentTypes.forEach((documentType) => {
    ...
    const documentOverride = (overrideArgs.documentTypes as any)?.[
      documentTypeName
    ] as WesjetOverrideDocumentType<string> | undefined;
    if (
      ...
      patchDocumentType(
        documentType,
        pick(documentOverride, ["filePathPattern", "computedFields"])
      );
    }
...
const patchDocumentType = (
  ...
  documentType.def = defineDocumentType(() =>
    mergeDeep({ ...previousDef, ...patch })
  ).def;
};
...
const patchNestedType = (
  ...
  nestedType.def = defineNestedType(() =>
    mergeDeep({ ...previousDef, ...patch })
  ).def;
};

const validateStackbitConfig = (
  stackbitConfig: Stackbit.Config | Stackbit.YamlConfig
): Stackbit.Config => {
...
const isImageModel = (model: Stackbit.Model): model is Stackbit.ImageModel =>
  model.type === "image";
