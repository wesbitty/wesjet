...
export type SourcePluginType = LiteralUnion<
  "local" | "contentful" | "sanity",
  string
>;
 ...
 * ```
 */
export type MarkdownUnifiedBuilderCallback = (
  builder: unified.Processor
) => void;
...
export type MDXBundlerMDXOptions =
  mdxBundler.BundleMDXOptions<any>["mdxOptions"];
...
export const processArgs = async <TArgs extends PartialArgs>(
  ...
  const {
    ...
    typeof argsOrArgsThunk === "function"
      ? await argsOrArgsThunk()
      : argsOrArgsThunk;
  ...
  const options: PluginOptions = {
    ...
    fieldOptions: {
      bodyFieldName:
        fieldOptions?.bodyFieldName ?? defaultFieldOptions.bodyFieldName,
      typeFieldName:
        fieldOptions?.typeFieldName ?? defaultFieldOptions.typeFieldName,
    },
