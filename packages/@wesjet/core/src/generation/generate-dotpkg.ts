...
import type { E, HasClock, HasConsole } from "@wesjet/utils/effect";
import { Array, Chunk, flow, OT, pipe, S, T } from "@wesjet/utils/effect";
...
export const logGenerateInfo = (
  info: GenerateInfo
): T.Effect<HasConsole, never, void> =>
  T.log(`Compiled ${info.documentCount} Files into .wesjet/jetpack`);
      ...
      pipe(
        ...
        S.mapEffectEitherRight((cache) =>
          pipe(
            ...
            T.eitherMap(() => ({
              documentCount: Object.keys(cache.cacheItemsMap).length,
            }))
          )
        )
      )
...
const writeFilesForCache = ({
  ...
  never,
  E.Either<
    fs.WriteFileError | fs.MkdirError | fs.RmError | fs.JsonStringifyError,
    void
  >
> =>
  pipe(
    T.gen(function* ($) {
      const withPrefix = (...path_: string[]) =>
        filePathJoin(targetPath, ...path_);
      ...
      const assertStatement = needsJsonAssertStatement
        ? ` assert { type: 'json' }`
        : "";

      const typeNameField =
        generationOptions.options.fieldOptions.typeFieldName;
      ...
      const individualDataJsonFiles = allCacheItems.map(
      ...
      const collectionDataJsonFiles = pipe(
        ...
        Array.map((documentDef) => {
          const documents = allDocuments.filter(
            (_) => _[typeNameField] === documentDef.name
          );
      ...
      const dataDirPaths = documentDefs.map((_) =>
        withPrefix("jetpack", _.name)
      );
      yield* $(
        T.forEachPar_([withPrefix("jetpack"), ...dataDirPaths], fs.mkdirp)
      );
      ...
      yield* $(
        T.collectAllPar([
          writeFile({
            filePath: withPrefix("package.json"),
...
`;
  ...
  const makeVariableName = flow(idToFileName, (_) =>
    camelCase(_, { stripRegexp: /[^A-Z0-9\_]/gi })
  );

  const docImports = documentIds
    .map(
      (_) =>
        `import ${makeVariableName(_)} from './${idToFileName(
          _
        )}.json'${assertStatement}`
    )
    .join("\n");
...
export const ${dataVariableName} = [${documentIds
    .map((_) => makeVariableName(_))
    .join(", ")}]
`;
};
...
const makeIndexMjs = ({
  ...
  const constExports =
    "export { " +
    dataVariableNames.map((_) => _.dataVariableName).join(", ") +
    " }";
...
export const makeDataTypes = ({
  ...
  const dataConsts = Object.values(schemaDef.documentTypeDefMap)
    .map(
      (docDef) =>
        [docDef, docDef.name, getDataVariableName({ docDef })] as const
    )
    .map(
...
const getDataVariableName = ({
  ...
  if (docDef.isSingleton) {
    ...
    return (
      "all" + utils.uppercaseFirstChar(utils.inflection.pluralize(docDef.name))
    );
  }
};
