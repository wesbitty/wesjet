    ...
    pipe(
      T.gen(function* ($) {
        if (process.platform === "win32") {
          yield* $(
            T.log(
              "Warning: .wesjet[jetpack] might not work as expected on Windows"
            )
          );
        }

        const result = yield* $(
          pipe(eff, provideTracing(tracingServiceName), provideCwd, T.result)
        );
...
https://github.com/wesbitty/wesbitty/discussions/3`)
            );
          }
          ...
          // otherwise for unmanaged errors or with `--verbose` flag provided, print the entire stack trace
            ...
            const wesjetVersion = yield* $(
              getWesjetVersion()["|>"](T.provide(DummyTracing))
            );
