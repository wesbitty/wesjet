...
export const provideSchema = async ({
  ...
  const nestedTypes = types.filter(
    (_): _ is Sanity.ObjectType => _.type === "object"
  );
  ...
  const usedObjectTypes = collectUsedObjectTypes({
    documentTypes,
    nestedTypes,
  });
...
const collectUsedObjectTypes = ({
  ...
  const sanityObjectTypeMap: { [typeName: string]: Sanity.ObjectType } =
    nestedTypes.reduce((acc, _) => ({ ...acc, [_.name]: _ }), {});
...
const sanityDocumentTypeToCoreDocumentDef =
  ...
  (documentType: Sanity.DocumentType): core.DocumentTypeDef => {
    ...
    return {
      ...
      fieldDefs: documentType.fields.map(
        sanityFieldToCoreFieldDef(objectTypeNames)
      ),
      computedFields: [],
      extensions: {},
    };
...
const sanityObjectTypeToCoreObjectDef =
  ...
  (objectType: Sanity.ObjectType): core.NestedTypeDef => {
    return {
      ...
      fieldDefs: objectType.fields.map(
        sanityFieldToCoreFieldDef(objectTypeNames)
      ),
...
const sanityFieldToCoreFieldDef =
  ...
  (field: Sanity.Field): core.FieldDef => {
    const isRequired = field.validation?.(sanityMockRule) as
      | boolean
      | undefined;
    const baseFields = {
      ...pick(field, ["description", "name"]),
      isRequired,
      isSystemField: false,
    };
    switch (field.type) {
      ...
      case "array":
        ...
        if ((field.options as any)?.list) {
          return <core.ListFieldDef>{
            ...baseFields,
            type: "list",
            of: <core.ListFieldDefItem.ItemEnum>{
              type: "enum",
              options: (field.options as any).list,
            },
          };
        }
        ...
        if (field.of.length === 1) {
          return <core.ListFieldDef>{
            ...baseFields,
            type: "list",
            of: sanityArrayOfToCoreFieldListDefItem(objectTypeNames)(
              field.of[0]
            ),
          };
        }
        ...
        return <core.ListPolymorphicFieldDef>{
          ...
          of: field.of.map(
            sanityArrayOfToCoreFieldListDefItem(objectTypeNames)
          ),
          typeField: "_type",
          default: undefined,
        };
...
const sanityArrayOfToCoreFieldListDefItem =
  ...
  (arrayOf: Sanity.ArrayOf): core.ListFieldDefItem.Item => {
    switch (arrayOf.type) {
      ...
      case "reference":
        return <core.ListFieldDefItem.ItemDocumentReference>{
          type: "reference",
          // TODO support polymorphic references
      ...
      default:
        if (objectTypeNames.includes(arrayOf.type)) {
          return <core.ListFieldDefItem.ItemNested>{
            type: "nested",
            nestedTypeName: arrayOf.type,
          };
        }
