  ...
  return {
    ...
    fieldDefs: contentType.fields.map((field: any) =>
      toFieldDef({
        ...
        schemaOverrides,
        fieldOverrides:
          schemaOverrides.documentTypes[contentType.sys.id]!.fields[field.id],
      })
    ),
    computedFields: [],
  ...
  return {
    ...
    fieldDefs: contentType.fields.map((field: any) =>
      toFieldDef({
        ...
        schemaOverrides,
        fieldOverrides:
          schemaOverrides.nestedTypes[contentType.sys.id]!.fields[field.id],
      })
    ),
    description: contentType.description,
    // labelField: contentType.displayField,
        ...
        case "Entry":
          ...
          if (isDocument({ schemaOverrides, contentTypeId })) {
            const documentTypeName =
              schemaOverrides.documentTypes[contentTypeId]!.defName;
            return {
              ...fieldBase,
              type: "reference",
              documentTypeName,
            ...
            const nestedTypeName =
              schemaOverrides.nestedTypes[contentTypeId]!.defName;
      ...
      if (field.items.type === "Link") {
        if (field.items.validations![0]!.linkContentType!.length === 1) {
          ...
          return {
            ...
            of: field.items.validations![0]!.linkContentType!.map(
              (contentTypeId) =>
