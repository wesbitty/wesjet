...
const getDataForFieldDef = ({
  ...
  T.gen(function* ($) {
    ...
    switch (fieldDef.type) {
      case "nested":
        const nestedTypeDef =
          schemaDef.nestedTypeDefMap[fieldDef.nestedTypeName]!;
        return yield* $(
          makeNestedDocument({
      ...
      case "string":
        ...
        if (rawFieldDataIsAsset(rawFieldData)) {
          const asset = allAssets.find(
            (_) => _.sys.id === rawFieldData.sys.id
          )!;
...
const rawFieldDataIsAsset = (
  rawFieldData: any
): rawFieldData is ContentfulAssetEntry =>
...
const getDataForListItem = ({
  ...
  if (rawItemData.sys?.id) {
    ...
    const typeName =
      schemaOverrides.nestedTypes[entry.sys.contentType.sys.id]!.defName;
  ...
  throw new Error(
    `Case unhandled. Raw data: ${JSON.stringify(rawItemData, null, 2)}`
  );
};
