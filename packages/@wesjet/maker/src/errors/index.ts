...
import { errorToString, pattern } from "@wesjet/utils";
...
export namespace FetchDataError {
    ...
    implements AggregatableError
  {
    ...
    renderLine = () =>
      `"${this.documentFilePath}" failed with ${errorToString(this.error)}`;
  }
...
`;
    ...
    renderLine = () =>
      `${this.documentFilePath} (Used type name: "${this.documentTypeName}")`;
  }
...
`;
  ...
  export class ExtraFieldDataError
    extends Tagged("ExtraFieldDataError")<{
      readonly documentFilePath: RelativePosixFilePath;
      readonly extraFieldEntries: readonly (readonly [
        fieldKey: string,
        fieldValue: any
      ])[];
      readonly documentTypeDef: core.DocumentTypeDef;
...
File paths have to be relative to \`contentDirPath\`: "${contentDirPath}")`;
  ...
  export class IncompatibleFieldDataError
    extends Tagged("IncompatibleFieldDataError")<{
      readonly documentFilePath: RelativePosixFilePath;
      readonly incompatibleFieldData: readonly (readonly [
        fieldKey: string,
        fieldValue: any
      ])[];
      readonly documentTypeDef: core.DocumentTypeDef;
...
Couldn't find a document for ${errorCount} singleton document types`;
    ...
    renderLine = () => {
      const filePathInfo = this.filePath
        ? ` at provided file path "${this.filePath}"`
        : ``;
...
This is possibly a bug in wesjet. Please open an issue.`;
    ...
    renderLine = () =>
      `"${this.documentFilePath}": ${errorToString(this.error)}`;
  }
}
...
export class DuplicateBodyFieldError extends Tagged("DuplicateBodyFieldError")<{
  ...
  toString = () =>
    `You cannot override the "${this.bodyFieldName}" field in a document definition.`;
}
