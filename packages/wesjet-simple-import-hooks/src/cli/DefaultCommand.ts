...
const defaultStackbitYamlPath = () =>
  `${path.join(process.cwd())}/stackbit.yaml`;
const defaultTransformPath = () =>
  `${path.join(process.cwd())}/wesjet-simple-import-hooks.js`;
...
`;
            ...
            console.log(
              `Stackbit config generated to ${this.stackbitYamlPath}`
            );
          },
          (error) => error
        )
...
const getTransform = async (
  ...
  if (typeof transform !== "function") {
    throw new Error(
      `Transform file "${transformPath}" doesn't export a function`
    );
  }
