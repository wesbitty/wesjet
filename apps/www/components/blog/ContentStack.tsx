      ...
      <div className="mb-4 flex flex-col items-center justify-around sm:h-12 sm:flex-row">
        {icons.map((name, idx) => {
          return (
            <span
              key={idx}
              className={`svg-h-full m-4 mb-0 block h-12 shrink-0 ${
                className ? className : ""
              }`}
            >
              <Icon name={name} />
...
const DecoupledStack: React.FC<Exclude<Props, "decoupled">> = ({
  content,
  processor,
  pages,
}) => {
  return (
    <div
      className={`flex flex-col items-center justify-between text-center md:flex-row`}
    >
      ...
      <PlusIcon />
      <div
        className={`p-8 ${borderClasses} flex flex-col items-center justify-between text-center sm:flex-row`}
      >
        <StackItem {...processor} />
        <PlusIcon />
        <StackItem {...pages} />
...
const MonolithicStack: React.FC<Exclude<Props, "decoupled">> = ({
  content,
  processor,
  pages,
}) => {
  return (
    <div className={`flex justify-center`}>
      <div
        className={`flex flex-col items-center justify-between p-8 text-center sm:flex-row ${borderClasses}`}
      >
        <StackItem {...content} />
        <PlusIcon />
        <StackItem {...processor} />
