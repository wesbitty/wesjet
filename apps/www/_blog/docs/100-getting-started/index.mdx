...
<div className="mt-8 grid gap-6 md:grid-cols-2">
  <Card title="Try it Now" icon="gitpod" link={{url: 'http://gitpod.io/#https://github.com/wesbitty/wesjetpkg/tree/master/examples/wesjet-starter', label: 'Try in Gitpod'}}>
    If you'd like to dive right in and start playing with Wesjet, you can open the project in Gitpod.

  </Card>
  <Card title="Example Project" icon="github" link={{url: 'https://github.com/wesbitty/wesjetpkg/tree/master/examples/wesjet-starter', label: 'See example'}}>
    You can find a working version of this tutorial in [this state](https://github.com/wesbitty/wesjetpkg/tree/master/examples/wesjet-starter/tree/bca4b7884f5cad8963e631a2f24a19200c106495) of the example project.
...
```txt
npx create-next-app wesjet-starter-example
```

That command will place the project in a `wesjet-starter-example` directory. Change into that directory.

```txt
cd wesjet-starter-example
```
...
Wesjet compiles files in the `.wesjet/jetpack` directory. This tells JavaScript (or TypeScript) to create an alias of `wesjet/jetpack` to the compiled files directory.
...
This configuration specifies a single document type called `Post`. These documents are expected to be `.md` files that live within a `posts` directory in your project. And data objects compiled from these files will have the following properties:
